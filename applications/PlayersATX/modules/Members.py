...
class Members:
    ...
    def export_members(self):
        ...
        attendance = self.get_current_attendance(searchDate=yesterday)
        if attendance["count"]:
            memberRows = self.db().select(self.db.members.ALL)
            if memberRows:
                members = []
                for memberRow in memberRows:
                    thisMember = {
                        "member_number": memberRow.member_number,
                        "expiration": memberRow.expiration,
                        "his_fName": memberRow.his_f_name,
                        "his_lName": memberRow.his_l_name,
                        "his_email": memberRow.his_email,
                        "his_dob": memberRow.his_dob,
                        "his_dl": memberRow.his_dl,
                        "her_fName": memberRow.her_f_name,
                        "her_lName": memberRow.her_l_name,
                        "her_email": memberRow.her_email,
                        "her_dob": memberRow.her_dob,
                        "her_dl": memberRow.her_dl,
                        "address": memberRow.address,
                        "city": memberRow.city,
                        "state": memberRow.state,
                        "zip": memberRow.zip,
                        "phone": memberRow.phone,
                        "member_type": self.db.membership_type(memberRow.status).membership_label if memberRow.status else "missing"
                    }
                    members.append(thisMember)

        return {"memberExport": members, "exportDate": yesterday}
