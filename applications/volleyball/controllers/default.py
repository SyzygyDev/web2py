...
def signup():
    failedLogin = request.vars.failed or None
    form1 = False
    form2 = False
    angularData = {"noData":True}
    if response.flash == auth.messages.invalid_login:
        angularData = {"login":"invalid email or password"}
    if auth.is_logged_in():
        ...
        # if auth.settings.on_failed_authentication = redirect(URL('default', 'signup', vars=dict(failed="1")))
        form1=auth.register()
        form2=auth.login()
