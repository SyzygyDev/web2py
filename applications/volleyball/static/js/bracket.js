...
app.directive('ngbracket', ['data', 'layoutService',
    function(data, layoutService) {
        return {
            restrict: 'E',
            templateUrl: '/volleyball/static/tpls/bracket.html',
            replace: false,
            scope: {
                bracketData: '=',
                fixedScore: "@"
...
.directive('match', ['connectorService', 'layoutService', 'data', '$filter', 'highlight',
    function(connectorService, layoutService, data, $filter, highlight) {
        return {
            restrict: "E",
            scope: false,
            templateUrl: '/volleyball/static/tpls/match.html',
            replace: true,
            link: {
...
.directive('score', ['data',
    function(data) {
        return {
            ...
            templateUrl: '/volleyball/static/tpls/score.html',
            link: function(scope, el, attrs) {
                var fixedScore = scope.$parent.$parent.$parent.$parent.$parent.fixedScore || false;
                scope.editScore = function(event) {
                    if (scope.match.team1.id && scope.match.team2.id && !fixedScore) {
...
.directive('team', ['data', 'highlight',
    function(data, highlight) {
        return {
            restrict: 'E',
            templateUrl: '/volleyball/static/tpls/team.html',
            replace: true,
            scope: {
                team: '=',
                teamdetails: '=',
            ...
            link: function(scope, el, attrs) {
                ...
                scope.editTeam = function() {
                    if (data.getProperties().status === 'Not started' && scope.team.id && !noEdit) {
                        scope.status = 'editable';
                    }
                };
